...
// ecall
size_t nacl_keygen_in_seal(unsigned char *o_sealed, size_t cap_sealed_buf,
  ...
  try {
    ...
    auto st =
        sgx_seal_data(0, nullptr, sk.size(), (const unsigned char *)sk.data(),
                      sealed_len, seal_buffer);
    if (st != SGX_SUCCESS) {
