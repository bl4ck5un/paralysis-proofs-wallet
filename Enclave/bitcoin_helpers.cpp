...
//! generate script for deposit. Coins can be spent either by userPubkey after a
//! timeout, or by exchPubkey at any time \param userPubkey \param exchPubkey
//! \param lockTime
//! \return script
...
//! It first check if scriptPubkey is of the right format: OP_HASH160 <HASH>
//! OP_EQUAL It then checks hash(redeemScript) == HASH \param redeemScript
//! \param scriptPubKey
//! \return true or else
bool validate_redeemScript(const CScript &redeemScript,
  ...
  if (!scriptPubKey.IsPayToScriptHash(scriptHash)) {
    ...
    return equal(std::begin(scriptHash), std::end(scriptHash),
